...
struct http_header http_hdr_content_html = HTTP_HDR_CONTENT_HTML;
									...
									unsigned short status_code)
{
    ...
    response = db_malloc(sizeof(struct http_response));
    debug("Generate HTTP response %p.\n", response);
    //Set initial values for the response.
    response->connection = connection;
    response->status_code = status_code;
    response->status_line = NULL;
    response->headers = db_malloc(sizeof(struct http_header) * HTTP_N_RESPONSE_HEADERS);
    response->headers[0] = http_hdr_content_html;
    response->headers[1] = http_hdr_connection_close;
    response->headers[2] = http_hdr_server;
    response->headers[3].name = "Length";
    ...
    if (status_code != 200)
    {
		uri = db_malloc(sizeof(char)*9);
		os_sprintf(uri, "/%d.html", status_code);
		debug(" HTTP error condition, loading: %s.\n", uri);
	}
	else
	{
		uri = request->uri;
    ...
    if (!data_size)
    {
		...
		if (i < n_static_uris)
		{
			...
			//always be freed.
			buffer = db_malloc((int)data_size);
	...
	if (data_size)
	{
        ...
        response->headers[3].value = db_malloc(sizeof(char) * digits(data_size));
